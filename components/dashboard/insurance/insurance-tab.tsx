...
function categoryIcon(cat: InsuranceCategory) {
  switch (cat) {
    ...
    case "disability": return faPersonCane;
    case "umbrella":   return faUmbrella;
    case "travel":     return faPlaneDeparture;
    case "pet":        return faPaw;
    ...
    <div className="flex items-start justify-between gap-4 py-3.5 border-b last:border-b-0 group">
      <div className="flex items-start gap-3 min-w-0">
        <div className={`mt-0.5 p-2 rounded-lg shrink-0 ${
          status === "expired"       ? "bg-red-50 dark:bg-red-950/30" :
          status === "expiring_soon" ? "bg-amber-50 dark:bg-amber-950/30" :
                                       "bg-muted"
        }`}>
          <FontAwesomeIcon
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {status === "expired" && (
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-red-600 border-red-200 bg-red-50 dark:bg-red-950/30 gap-1">
                <FontAwesomeIcon icon={faCircleXmark} className="h-2.5 w-2.5" /> Expired
              </Badge>
            )}
            {status === "expiring_soon" && (
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-amber-600 border-amber-200 bg-amber-50 dark:bg-amber-950/30 gap-1">
                <FontAwesomeIcon icon={faTriangleExclamation} className="h-2.5 w-2.5" /> Renews in {days}d
              </Badge>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground flex-wrap">
            ...
            {status === "ok" && renewal && (
              <span className="flex items-center gap-1">
                ...
                Renews {new Date(policy.renewal_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
              </span>
            )}
            {policy.notes && (
      ...
      <div className="flex items-start gap-2 shrink-0">
        <div className="text-right">
          ...
          <p className="text-xs text-muted-foreground">{formatCurrency(policy.premium_monthly * 12)}/yr</p>
        </div>
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          <FontAwesomeIcon icon={faArrowTrendUp} className="h-3.5 w-3.5 text-muted-foreground" />
          Premium by category
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="flex justify-between text-xs pt-0.5">
          ...
          <span className="font-semibold tabular-nums">{formatCurrency(totalMonthly)}</span>
        </div>
      </CardContent>
    </Card>
  );
...
function RenewalTimelineCard({
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          <FontAwesomeIcon icon={faCalendarDay} className="h-3.5 w-3.5 text-muted-foreground" />
          Renewal timeline
        </CardTitle>
      </CardHeader>
        ...
        {renewals.map((r) => {
          ...
          const label = isExpired
            ...
            : isSoon
              ...
              : new Date(r.renewalDate).toLocaleDateString("en-US", { month: "short", year: "numeric" });

          return (
            <div key={r.policy_id} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <FontAwesomeIcon icon={categoryIcon(r.category)} className="h-3 w-3 text-muted-foreground shrink-0" />
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {isExpired
                  ...
                  : isSoon
                    ? <FontAwesomeIcon icon={faTriangleExclamation} className="h-3 w-3 text-amber-500" />
                    : <FontAwesomeIcon icon={faCircleCheck} className="h-3 w-3 text-emerald-500" />
                }
                <span className={`text-xs tabular-nums ${
                  isExpired ? "text-red-600" : isSoon ? "text-amber-600" : "text-muted-foreground"
                }`}>
                  {label}
                </span>
              </div>
...
const emptyDraft: PolicyDraft = {
  name: "", category: "", provider: "", policy_number: "",
  coverage_amount: "", premium_monthly: "", deductible: "",
  start_date: new Date().toISOString().split("T")[0],
  renewal_date: "", auto_renew: true, beneficiary: "", notes: "",
};
...
function PolicyFormDialog({
  ...
  const set = (field: keyof PolicyDraft) => (e: React.ChangeEvent<HTMLInputElement>) =>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4 py-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5 col-span-2">
              ...
              <Input placeholder="e.g. Term Life Insurance" value={draft.name} onChange={set("name")} />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setDraft((d) => ({ ...d, category: v as InsuranceCategory }))}
              >
            ...
            <div className="space-y-1.5">
              ...
              <Input placeholder="e.g. MetLife" value={draft.provider} onChange={set("provider")} />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input type="number" min="0" className="pl-7" placeholder="0" value={draft.premium_monthly} onChange={set("premium_monthly")} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">$</span>
                <Input type="number" min="0" className="pl-7" placeholder="0" value={draft.coverage_amount} onChange={set("coverage_amount")} />
              </div>
            </div>

            <div className="space-y-1.5">
              <Label>Start date</Label>
              <Input type="date" value={draft.start_date} onChange={set("start_date")} />
            </div>

            <div className="space-y-1.5">
              <Label>Renewal date</Label>
              <Input type="date" value={draft.renewal_date} onChange={set("renewal_date")} />
            </div>

            <div className="space-y-1.5">
              <Label>Policy number</Label>
              <Input placeholder="Optional" value={draft.policy_number} onChange={set("policy_number")} />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">$</span>
                <Input type="number" min="0" className="pl-7" placeholder="0" value={draft.deductible} onChange={set("deductible")} />
              </div>
            </div>

            <div className="space-y-1.5 col-span-2">
              <Label>Beneficiary (optional)</Label>
              <Input placeholder="e.g. Spouse" value={draft.beneficiary} onChange={set("beneficiary")} />
            </div>

            <div className="space-y-1.5 col-span-2">
              <Label>Notes (optional)</Label>
              <Input placeholder="Any context or reminders…" value={draft.notes} onChange={set("notes")} />
            </div>
            ...
            <div className="flex items-center justify-between col-span-2">
              ...
              <button
                ...
                onClick={() => setDraft((d) => ({ ...d, auto_renew: !d.auto_renew }))}
                className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${draft.auto_renew ? "bg-primary" : "bg-muted"}`}
              >
                <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${draft.auto_renew ? "translate-x-4" : "translate-x-1"}`} />
              </button>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!draft.name.trim() || !draft.category || !draft.renewal_date}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <p className="text-sm text-muted-foreground">
          Are you sure you want to remove <span className="font-medium text-foreground">{policy?.name}</span>? This cannot be undone.
        </p>
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button>
...
export function InsuranceTab() {
  const [policies, setPolicies] = React.useState<InsurancePolicy[]>(mockInsurancePolicies);
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<InsurancePolicy | null>(null);
  ...
  const kpiItems: KpiItem[] = [
    {
      ...
      value: String(insuranceSummary.totalPolicies + properties.flatMap((p) => p.insurance).length),
    ...
    {
      label: "Expiring Soon",
      value: insuranceSummary.expiringSoonCount > 0 ? `${insuranceSummary.expiringSoonCount} within 60d` : "None",
      subline: insuranceSummary.expiringSoonCount > 0 ? "Action may be required" : "All renewals on schedule",
      tone: insuranceSummary.expiringSoonCount > 0 ? "warning" : "good",
    },
    {
      label: "Expired",
      value: insuranceSummary.expiredCount > 0 ? `${insuranceSummary.expiredCount} expired` : "None",
      subline: insuranceSummary.expiredCount > 0 ? "Needs immediate attention" : "All policies current",
  ...
  function handleDeleteConfirm() {
    ...
    setPolicies((prev) => prev.filter((p) => p.policy_id !== deleteTarget.policy_id));
    setDeleteOpen(false);
